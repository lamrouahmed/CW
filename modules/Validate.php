            ...
            foreach ($fields as $field => $rules) {
                foreach ($rules as $ruleName => $rule) {
                    ...
                    if(empty($value) && $ruleName === "name") {
                        $this->setError("entrez un(e) {$rules["name"]}", $field);
                    } else if(!empty($value)) {
                        switch ($ruleName) {

                            case "min":
                                if(strlen($value) < $rule) $this->setError("{$rules["name"]} doit avoir au moins {$rule} characteres", $field);
                                break;

                            case "max":
                                if(strlen($value) > $rule) $this->setError("{$rules["name"]} doit avoir au plus {$rule} characteres", $field);
                                break;

                            case "mail":
                                if(!filter_var($value, FILTER_VALIDATE_EMAIL)) $this->setError("{$rules["name"]} invalide", $field);
                                break;

                            case "match":
                                if(Input::get($field) !== Input::get($rule)) $this->setError("mot de passe n'est pas le même", $field);
                                break;
                            ...
                            case "unique":
                                ...
                                {
                                    $this->setError("{$rules["name"]} existe déjà", $field);
                                }
                                break;
                            case "update":
                            ...
                            case "exists":
                                ...
                                {
                                    $this->setError("{$rules["name"]} n'existe pas", $field);
                                }
                            break;
                            case "date":
                            ...
                            case "credit_card":            
                                    ...
                                    // }   
                                    $error = 0;
                                    foreach ($regExps as $bank => $regExp) {
                                        if(preg_match($regExp, str_replace(' ', '', $rules[$ruleName]))) {
                                            $error = 1;
                                        }
                                    }
                                    if(!$error) {
